  ...
  include Mongoid::Document
	...
	def send_contribution_mail
		...
		if self.project_id #There is a project owner to send this email to
			...
			email = ItemContributionMailer.new(:admin_email => admin_email, :item_name => self.item_name, :amount => self.amount, :project_query => self.project_query, :project_date => self.start_at.to_date, :contributor_email => self.contributor_email, :contributor_name => self.contributor_name)
			email.deliver		
	...
	def send_retreat_contribution_mail
		...
		if self.project_id #There is a project owner to send this email to
			admin_email = Project.find(self.project_id).owner.email #Get owner email
			email = ItemRetreatContributionMailer.new(:admin_email => admin_email, :item_name => self.item_name, :amount => self.amount, :project_query => self.project_query, :project_date => self.start_at.to_date, :contributor_email => self.contributor_email, :contributor_name => self.contributor_name)
			email.deliver		
		elsif item.owner #There is an item owner to send this email to
			admin_email = item.owner.email #Get owner email
